...
export const useProductStore = create<ProductsState>((set, get) => ({
    ...
    set((state) => ({
        ...state, options: data, status: filters.status }));
    },
    getFiltersByCategory: async (categorie) => {
        const data = await fetch(`https://fakestoreapi.com/products/category/${categorie}`)
        const products = await data.json();
        set((state) => ({ ...state, products, status:data.status }));
    }
}));
