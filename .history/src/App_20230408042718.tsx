...
import { useProductStore } from "./store/counterStore";
...
function App() {
  ...
  const { title, products, filter } = useProductStore(
    (state) => ({
      title: state.title,
  ...
  return (
    <div className="md:container md:mx-auto ">
      ...
      {
        isAlertVisible || isAlertError && (
          <div className={`fixed bottom-0 
          ${isAlertError ? 'bg-red-100  border-red-500' : "bg-teal-100  border-teal-500 "} 
          border-t-4 rounded-b text-teal-900 px-4 py-3 shadow-md`} role="alert">
          <div className="flex">
            <div>
                <p className="font-bold">{ isAlertError ? "Oops Something" : "Page Load in time"}.</p>
            </div>
          </div>
        </div>
        )
